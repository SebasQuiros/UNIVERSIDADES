...
export default function ExerciseWorkspacePage() {
  ...
  return (
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      {/* El encabezado con badges + título del ejercicio se eliminó (era redundante
          con el breadcrumb del TopBar y se veía plano). El contexto útil —empresa y
          nota— vive ahora en el encabezado de sección de abajo. */}
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {company && (
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <span className="text-sm text-gray-700 truncate max-w-[180px]">{company.name}</span>
                </div>
              )}
              {attempt.status === 'GRADED' && attempt.score != null && (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl cx-tada">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                  <span className="text-sm font-bold text-emerald-700 tabular-nums">{attempt.score} / {attempt.maxScore} pts</span>
                </div>
              )}
              {activeTab !== 'dashboard' && (
                <button onClick={() => setActiveTab('dashboard')}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-700 hover:text-blue-800 cx-press">
                  <ArrowLeft className="w-3.5 h-3.5" /> Resumen
                </button>
              )}
            </div>
          </div>
